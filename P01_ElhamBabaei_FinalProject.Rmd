---
title: "P01- SNA_FinalProject - 2022"
output:
...
In P01 file, we have 300 authors and 4732 events. The incidence matrix shows a two mode network, author-paper relationship.
...
incmat <- read.csv("P01_IncMat.csv",header = T, row.names = 1)
...
We change the diagonal of adjacency matrix to values zero to avoid circle edges to each node itself because in this case We cannot obtaing nice visualizations. 
...
nodeInfo <- read.csv("P01_NodeAttr.csv",header=TRUE,stringsAsFactors=FALSE, sep=",")
nodeInfo
```
...
```
```{r}
hist(deg, xlab="degree", main="P01- Degree distribution", prob=F, col = "light blue")
```
...
gplot(net, vertex.cex=sqrt(bet)/25, gmode="graph", displaylabels = F, 
      label.cex = 0.8, main="P01- Network \n Node size: sqrt(betweennes)/25") 
```
...
ev <- sna::evcent(net) 
ev
hist(ev, main = "P01- Eigenvector Centrality", col="light green")
...
tc
barplot(tc, names.arg=colnames(tc), las=2, col=7, main = "P01- Triadic census")
...
There are 45 connected components.
``` {r}
#Components 
...
gplot(net[cl,cl], boxed.lab=FALSE, label.cex=0.5,label.col=4, displaylabels = F, gmode = "graph",
      label=network.vertex.names(net)[cl], main="P01- The largest connected component")
...
The distribution of edge weights is very skewed and shows that the pair of authors usually contribute in less than 5 papers together. The maximum weight is 54.
```{r}
...
hist(E(mygraph)$weight, col="lightblue", xlab="Edge weight", ylab="Frequency", 
     main="P01- Edge weight Distribution")
...
plot(k.1, vertex.label.cex=0.8, vertex.cex=0.5 , 
     main=" P01- Neighborhood of order 1 for 'Adelchi Azzalini' ")
plot(k.1.2, vertex.label.cex=0.8, vertex.cex=0.5, 
     main=" P01- Neighborhood of order 2 for 'Adelchi Azzalini' ")
...
hist(graph.strength(mygraph), col="pink", xlab="Vertex Strength", ylab="Frequency",
     main="P01- Vertex strength distribution") 
```
Edge betweennes is defined by the number of geodesic distances (shortest paths) going through that edge.

The result shows that the statistician `Francesco Bartolucci` plays the most important role in terms of edge betweeness in the graph. This shows that he has many connection with other authors.
```{r}
# Edge betweennes 
...
There are 995 cliques of sizes from 1 to maximum 6.
```{r}
#cliques
...
We look at cliques of size 6

```{r}
which(sapply(cliques(mygraph), length)==6)  # which cliques of order 6
cliques.mygraph[945]
cliques.mygraph[947]
```

```{r}
# the induced subgraph created by the nodes of clique of order 6
plot(induced.subgraph(mygraph, cliques.mygraph[[945]]), vertex.cex=2, vertex.label.cex=0.8,
     main="P01- The largest clique of size 6")  
plot(induced.subgraph(mygraph, cliques.mygraph[[947]]), vertex.cex=2, vertex.label.cex=0.8,
     main="P01- The largest clique of size 6")
...
ego.instr <- induced.subgraph(mygraph, neighborhood(mygraph, order= 1, nodes= 227)[[1]]) 
#neighborhood of node number 227 (or we can use the label of node, node="Nicola TORELLI") 
#order is the same geodesic distance                                                                  
edge_density(mygraph)
edge_density(ego.instr)

plot(ego.instr, vertex.cex=0.3, vertex.label.cex=0.8, main="P01- Induced subgraph for 'Nicola Torelli'")   
```
The authors `Nicola TORELLI`, `Matilde TREVISANI`, `Francesco PAULI`, and `Gabriella SCHOIER` are all from the University of Trieste.

Then look at the local transitivity for these authors.
```{r}
#transitivity
...
transitivity(mygraph, "local", vids=c(227,206, 111, 116))
transitivity(mygraph, "local", vids=c("Nicola TORELLI", "Matilde TREVISANI", 
                                      "Francesco PAULI", "Gabriella SCHOIER"))
...
The number of connected components is the same 45 that we obtained using the Network package.
...
gplot(adjmat, gmode="graph",displaylabels = F, label.cex = 0.7, edge.lwd=adjmat, main="P01-Network") 
...
print("P01-The first five pair of authors with highets co-authorship")
...
title(" P01- Fruchterman & Reingold layout\n Node size = node strength")
legend('bottomleft', fill=c("orange","lightblue", "red", "blue", "black"),
       legend=c("Femal (F)","Male (M)", "F-F co-outhorship",
                "M-M co-outhorship",
                "F-M co-outhorship"),cex=0.75)
...
V(mygraph)$label.color <- "blue"
plot(mygraph, layout=layout_with_fr, vertex.label = " ",  vertex.label.dist=0, vertex.label.cex=0.6) 
title(main="P01- Fruchterman & Reingold layout layout.\n Node size = betweenness centrality", cex.main=1)
legend('bottomleft', fill=c("orange","lightblue", "red", "blue", "black"),
       legend=c("Femal (F)","Male (M)", "F-F co-outhorship",
                "M-M co-outhorship",
                "F-M co-outhorship"),cex=0.75)
...
plot(dd, pch=19, cex=0.8, col="orange", xlab="Degree", ylab="Cumulative", main = "P01- Cumulative degree distribution")
```
...
title(main="P01- heatmap plot ", cex.main=0.8)
```
...
title(main="P01- heatmap plot by gender ", cex.main=0.8)
```
...
```
The following two chunks give an error because of lack of space. I comment it to be able to knit to pdf/HTML.
...
The following chunk shows the number of nodes in each of 45 connected components.
...
# plot
plot(mygraph, layout=ll, vertex.size=6, vertex.label=" ", vertex.label.cex=0.8, main='P01-Coreness')
...
maxCoreness     # so k-core is actually 5-core
verticesHavingMaxCoreness <- which(coreness == 5) 
...
plot(kcore,  vertex.label=igraph::get.vertex.attribute(kcore,name='vert.names',index=V(kcore)), 
     vertex.size=5,vertex.cex=2, vertex.label.cex=0.8, main="P01- Vertices having coreness 5")

```
With Girvan-newman algorithm we obtain 59 clusters.

Cluster number 24 has the maximum number of members which is 25.
```{r}
...
plot(gn.comm, mygraph, vertex.size = 6, vertex.color=colors[membership(gn.comm)], 
     vertex.label = NA, edge.width = 1, main= "P01- Clusters from Girvan-Newman algorithm")
```
...
gini(V(mygraph)$Role[V(mygraph)$GN.cluster==24])   # 24 is the largest cluster. output 1 means everyone in cluster number 24 belongs to the same community
```

gini index for cluster number 24 is not that high.
...
Louian algorith creates 64 clusters. The largest one is cluster number 27 with 20 members.
```{r}
# visualizing the result of dividing the network into communities
colors <- rainbow(max(membership(louv.clu)))
plot(louv.clu, mygraph, vertex.size = 6, vertex.color=colors[membership(louv.clu)], vertex.label = NA, edge.width = 1, main= "P01- Clusters from Louvain algorithm")
...
gini(V(mygraph)$Role[V(mygraph)$LOUV.cluster==27])   # 27 is the largest cluster. output 1 means everyone in cluster number 24 belongs to the same community
...
Girvan Newman algorithm gives 59 clusters with modularity 0.82, while Louvain algorithm provides 66 clusters and modularity 0.80.

So Girvan Newman algorithm is better in this case because of lower number of clusters and higher modularity. 
...
Considering k=5 for Clique Percolation algorithm, there are 3 nodes shared between two cliques.
```{r}
options(warn = -1)

cp.comm_w <-  cpAlgorithm(get.adjacency(mygraph, attr = "weight", sparse = F), k=5, method = "weighted", I=0.5) 
...
plot(ergm_induced_graph, vertex.label=NA, vertex.size=5, main="P01- Sub-graph for ERGM")
```
...
ergm_induced_net
plot(ergm_induced_net, main="P01- Sub-network for ERGM")
```
...
```
Note: Some chunks give an error  so they are commented out to kit to pdf.
```{r}
# goodness of fit
